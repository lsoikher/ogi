...
if ( ! class_exists( 'YITH_WooCommerce_Gift_Cards' ) ) {
	...
	class YITH_WooCommerce_Gift_Cards {
		...
		protected function __construct() {
			...
			//todo 2016.03.22
			add_action( 'woocommerce_order_status_completed', array( $this, 'generate_gift_card_number' ), 1 );

			/**
			 * Generate a valid card number for every gift card product in the order
			 */
			//add_action ( 'woocommerce_order_status_changed', array ( $this, 'generate_gift_card_number' ), 1, 3 );
		...
		public function enqueue_frontend_files() {
			...
			if ( 'gift-card' != $product->product_type ) {
				return;
			}

			//  register and enqueue ajax calls related script file
			wp_register_script( "ywgc-frontend",
				yith_minified_script_path( YITH_YWGC_URL . 'assets/js/ywgc-frontend' ),
				array(
					'jquery',
					'woocommerce',
				),
		...
		function enqueue_backend_files( $hook ) {
			...
			wp_register_script( "ywgc-backend",
				yith_minified_script_path( YITH_YWGC_URL . 'assets/js/ywgc-backend' ),
				array(
					'jquery',
					'jquery-blockui',
				),
		...
		private function show_gift_card_amount_list( $product_id ) {
			$amounts = ywgc_instance()->get_gift_card_product_amounts( $product_id );
			?>
		 ...
		 * @since  1.0.0
		 */
		public function generate_gift_card_number( $order_id/*$order, $old_status, $new_status */ ) {
			/*
						if ( is_numeric ( $order ) ) {
			...
			*/
			...
			foreach ( $order->get_items( 'line_item' ) as $order_item_id => $order_item_data ) {
				...
				$gift_card = ywgc_instance()->create_gift_card( $product_id, $order->id, $amount );
		...
		public function deduct_amount_from_gift_card( $id, $item_id, $code, $discount_amount, $discount_amount_tax ) {
			...
			if ( $gift != null ) {
				$gift->deduct_amount_from_gift_card( $discount_amount + $discount_amount_tax );
			}
		}
