...
if ( ! function_exists ( 'yith_minified_script_path' ) ) {
    /**
     * Return the path to a minified JS file, if exists
     *
     * @param string $script_path script path, without extension (no .js)
     *
     * @return string the path to the resource to use (returns the minified version if exists or the original path)
     */
    function yith_minified_script_path ( $script_path ) {
        $maintenance = isset( $_GET[ "script_debug_on" ] );
        $suffix      = ( defined ( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) || $maintenance ? '' : '.min';

        $return_path = $script_path . $suffix . '.js';;

        $source_path = $script_path . '.js';
