...
class InstapageCmsPluginPageModel
{
  ...
  public function update( $data )
  {
    ...
    $instapage_id = InstapageCmsPluginHelper::getVar( $data->landingPageId, null );
    $type = InstapageCmsPluginHelper::getVar( $data->type );
    $slug = InstapageCmsPluginHelper::getVar( $data->slug );
    $enterprise_url = InstapageCmsPluginConnector::getHomeURL();
  ...
  public function publishPage( $data )
  {
    ...
    $old_page_id = InstapageCmsPluginHelper::getVar( $data->id, null );
    $new_instapage_id = InstapageCmsPluginHelper::getVar( $data->landingPageId, null );
  ...
  public function migrateDeprecatedData( $data )
  {
    ...
    foreach( $data as $deprecated_page )
    {
      ...
      if( count( $landing_pages_by_id ) )
      {
        ...
        $raport[] = InstapageCmsPluginConnector::lang( 'Old version of page (slug: %s, Instapage ID: %s) is present in new database (slug: %s) and won\'t be migrated.', $deprecated_page->slug, $deprecated_page->landingPageId, $new_landing_page->slug );
      ...
      if( $landing_pages_by_slug )
      {
        ...
        $raport[] = InstapageCmsPluginConnector::lang( 'Slug: %s is already taken in new database. Old page (slug: %s, Instapage ID: %s) won\'t be migrated.', $deprecated_page->slug, $deprecated_page->slug, $deprecated_page->landingPageId );
      ...
      if( $landing_pages_by_type )
      {
        ...
        $raport[] = InstapageCmsPluginConnector::lang( 'One %s page already exists in new database. Old page (slug: %s, Instapage ID: %s) won\'t be migrated.', $deprecated_page->type, $deprecated_page->slug, $deprecated_page->landingPageId );

        continue;
      }

      if( $this->update( $deprecated_page ) ) {
        $raport[] = InstapageCmsPluginConnector::lang( 'Old version of page (slug: %s, Instapage ID: %s) successfully migrated.', $deprecated_page->slug, $deprecated_page->landingPageId );
      }
      else
      {
        $raport[] = InstapageCmsPluginConnector::lang( 'Old version of page (slug: %s, Instapage ID: %s) cannot be migrated due to database error.', $deprecated_page->slug, $deprecated_page->landingPageId );
      }
    }
  ...
  public function display( $page, $forced_status = null )
  {
    ...
    InstapageCmsPluginHelper::writeDiagnostics( $slug . ' : ' . $instapage_id, 'slug : instapage_id' );
    ...
    {
      $query_sufix = '?' . http_build_query( $_GET );
    }
    ...
    $cookie_string = @InstapageCmsPluginHelper::getVar( $result[ 'headers' ][ 'set-cookie' ], '' );
    $instapage_variant = InstapageCmsPluginHelper::getVariant( $cookie_string );
    ...
    {
      $status = InstapageCmsPluginHelper::getVar( $result[ 'response' ][ 'code' ], 200 );  
    }
