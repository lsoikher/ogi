...
class WJECF_Pro_Free_Products extends Abstract_WJECF_Plugin {
	...
	public function __construct() {    
		add_action('init', array( &$this, 'controller_init' ), 9); //Before WC class-wc-form-handler does it
	}
...
// ==================
	...
	public function filter_woocommerce_cart_item_quantity ( $product_quantity_html, $cart_item_key ) {		
		$version_ok = WJECF()->check_woocommerce_version('2.2.9');
    	if ( $version_ok ) {
...
/* ADMIN HOOKS */
	...
	public function admin_coupon_metabox_products( $thepostid, $post ) {
		...
		WJECF_ADMIN()->render_admin_product_selector( 'wjecf_free_product_ids', 'wjecf_free_product_ids', $free_product_ids, null );

        echo '<img class="help_tip" data-tip="' 
        . __( 'Free products that will be added to the cart when this coupon is applied.', 'woocommerce-jos-autocoupon' )
        . '" src="' . WC()->plugin_url() . '/assets/images/help.png" height="16" width="16" /></p>';
