...
// =============== FRONTEND HOOKS ==================== //
    ...
    public function woocommerce_coupon_get_discount_amount ( $discount, $discounting_amount, $cart_item, $single, $coupon ) {
        ...
        if ( $wrap_coupon->is_type( array( 'fixed_cart' ) ) || is_null( $cart_item ) ) {
            return $discount;
        }

        $apply_to = $wrap_coupon->get_meta( '_wjecf_apply_discount_to' );
        if ( empty( $apply_to ) ) {
            return $discount;
        }
        ...
        //echo $cart_item['data']->post->post_title . ' ' . WJECF_Wrap( $coupon )->get_code() . ' disc: ' .$discount. ' disc_am: ' .$discounting_amount. '<br>';
        // echo "<br>";
        ...
        if ( $wrap_coupon->get_limit_usage_to_x_items() === 0 ) {
            if (  $wrap_coupon->is_type( array( 'percent_product', 'percent' ) ) ) {
                $expected_discount = $wrap_coupon->get_amount() * ( $discounting_amount / 100 );
                $orig_discount_qty *= $expected_discount == 0 ? 0 : $discount / $expected_discount; 
            } elseif ( $wrap_coupon->is_type( 'fixed_product' ) ) {
                $expected_discount = min( $wrap_coupon->get_amount(), $discounting_amount );
        ...
        if ( $discount_qty != $orig_discount_qty ) {
            ...
            if ( $wrap_coupon->get_limit_usage_to_x_items() !== '' ) {
                $wrap_coupon->set_limit_usage_to_x_items( $wrap_coupon->get_limit_usage_to_x_items() + $orig_discount_qty - $discount_qty );
            }
        }
    ...
    private function get_discount_qty_every_nth_item( $cart_item, $discount_qty, $coupon ) {
        ...
        foreach( $discountable_cart_items as $key => $discountable_cart_item ) {
            if ( $discountable_cart_item == $cart_item ) {
                ...
                //$this->log( 'debug', sprintf('%s: discounting %s of %s idx: %d...%d  %s', WJECF_Wrap( $coupon )->get_code(), $discount_qty, $orig_discount_qty, $from_idx, $to_idx, $cart_item['data']->post->post_title ));
                break;
            } else {
