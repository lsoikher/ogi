...
class WJECF_Pro_Product_Filter extends Abstract_WJECF_Plugin {
    ...
    function assert_coupon_is_valid ( $coupon ) {
        $custom_fields = WJECF_Wrap( $coupon )->get_meta( '_wjecf_custom_fields' );

        if ( empty( $custom_fields ) ) {
            return;
        }

        $custom_fields_and = WJECF_Wrap( $coupon )->get_meta( '_wjecf_custom_fields_and' ) == 'yes';
    ...
    private function product_matches_custom_field( $product, $cf ) {
        ...
        $metavalue = WJECF_Wrap( $product )->get_meta( $cf['key'], true );

        //3.0.0: Verify if metavalue is a string
...
/** ADMIN **/
    ...
    public function on_woocommerce_coupon_options_usage_restriction() {
        ...
        $coupon_custom_fields = $wrap_coupon->get_meta( '_wjecf_custom_fields', true );
                ...
                <span class="wjecf-table" id="wjecf-table-custom-field">
                    <span class="tr">
                        ...
                        <span class="th"><?php _e( 'Value', 'woocommerce' ); ?>
                            <?php echo WJECF_Admin_Html::wc_help_tip( 
                                __(
