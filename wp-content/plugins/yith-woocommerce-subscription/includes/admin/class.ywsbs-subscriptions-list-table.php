...
class YITH_YWSBS_Subscriptions_List_Table extends WP_List_Table {
    ...
    public function __construct( $args = array() ) {
        ...
        $this->post_type = YWSBS_Subscription()->post_type_name;
    }
    ...
    function column_default( $item, $column_name ) {
        switch( $column_name ) {
            case 'id':
                ...
                break;
            case 'status':
                $status = get_post_meta( $item->ID, '_status', true);
                return $status;
                break;
            case 'post_title':
                $product_name = get_post_meta( $item->ID, '_product_name', true);
                $quantity = get_post_meta( $item->ID, '_quantity', true);
                $qty = ( $quantity > 1) ? ' x '. $quantity : '';
                return $product_name.$qty;
                break;
            case 'user':
                $user_id = get_post_meta( $item->ID, '_user_id', true);
                $user_data = get_userdata( $user_id );
                return '<a href="' . admin_url( 'profile.php?user_id=' . $user_id  ) . '">' . $user_data->user_nicename . '</a>';
                break;
            case 'recurring':
                $recurring = get_post_meta( $item->ID, '_line_total', true);
                $currency = get_post_meta( $item->ID, '_order_currency', true);
                return  wc_price( $recurring, array('currency' => $currency) );
                break;
            case 'order':
                $order_ids = get_post_meta( $item->ID, '_order_ids', true);
                $last_order = end( $order_ids );
                return  '#'.$last_order;
                break;
            case 'expired':
                $expired_date = get_post_meta( $item->ID, '_expired_date', true);
