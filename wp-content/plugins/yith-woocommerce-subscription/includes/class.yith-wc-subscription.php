...
if ( !class_exists( 'YITH_WC_Subscription' ) ) {
    ...
    class YITH_WC_Subscription {
        ...
        public function change_price_html( $price, $product ) {
            ...
            if( ! $this->is_subscription($product->id) ){
                return $price;
            }

	        $price_is_per      = get_post_meta( $product->id, '_ywsbs_price_is_per', true );
	        $price_time_option = get_post_meta( $product->id, '_ywsbs_price_time_option', true );
