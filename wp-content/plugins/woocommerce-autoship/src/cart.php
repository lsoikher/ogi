...
function wc_autoship_print_cart_autoship_options( $for_product = null ) {
	...
	$autoship_enabled = get_post_meta( $for_product->id, '_wc_autoship_enable_autoship', true );
	if ( $autoship_enabled == 'yes' ) {
		// Get autoship price
		$autoship_price = apply_filters( 'wc_autoship_price',
			get_post_meta( $for_product->id, '_wc_autoship_price', true ),
			$for_product->id,
			0,
			get_current_user_id(),
			0
		...
		$autoship_min_frequency = get_post_meta( $for_product->id, '_wc_autoship_min_frequency', true );
		$autoship_max_frequency = get_post_meta( $for_product->id, '_wc_autoship_max_frequency', true );
		$autoship_default_frequency = get_post_meta( $for_product->id, '_wc_autoship_default_frequency', true );
		// Render template
...
function wc_autoship_print_cart_autoship_options_variable( $for_product = null ) {
	...
	if ( $autoship_enabled == 'yes' ) {
		...
		$autoship_price = '';
		$default_attributes = $for_product->get_variation_default_attributes();
