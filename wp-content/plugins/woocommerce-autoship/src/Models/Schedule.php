...
// 		$this->_items = NULL;
	...
	public function get_coupon_expiration() {
		...
		if ( ! empty( $coupon ) ) {
			// Expiration date
			$expiration_date = get_post_meta( $coupon->id, 'wc_autoship_expiration_date', true );
	...
	public function get_coupon_discount_total() {
		...
		if ( ! empty( $coupon ) ) {
			...
			if ( ! $this->coupon_is_expired() ) {
				// Percent discount
				$percent_discount = get_post_meta( $coupon->id, 'wc_autoship_percent_discount', true );
	...
	public function get_shipping_rate() {
		...
		foreach ( $shipping_methods as $method ) {
			foreach ( $method->rates as $rate ) {
				if ( $rate->id == $customer->get_shipping_method() ) {
					$shipping_rate = $rate;
					break 2;
				}
			}
