...
class WC_Autoship_Pipey_Client {
	...
	public function login( $user_id ) {
		...
		$this->_post( admin_url( '/admin-ajax.php?action=wc_autoship_pipey_set_user' ), array( 'user_id' => $user_id ) );
		$this->empty_cart();
	}
	...
	public function add_item( $product_id, $variation_id = 0, $quantity = 1, $autoship_frequency = 0 ) {
		...
		$result = $this->_post( $this->_cart_url( '?wc-ajax=add-to-cart' ), $data );
	...
	public function apply_coupon( $coupon_code ) {
		...
		$this->_post( $this->_cart_url( '?wc-ajax=apply_coupon' ), $data );
		$this->_head( $this->_cart_url() );
	}
	...
	public function update_shipping_method( $shipping_method ) {
		...
		$this->_post( $this->_cart_url( '?wc-ajax=update_shipping_method' ), $data, array( 'Referer: ' . $nonce_data['referer'] ) );
