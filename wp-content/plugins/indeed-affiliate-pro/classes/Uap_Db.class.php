...
if (!class_exists('Uap_Db')){
	class Uap_Db{
		...
		public function create_tables(){
			...
			/// UAP_REF_LINKS
			$table_name = $wpdb->base_prefix . 'uap_ref_links';
		...
		public function check_update_notifications(){
			 ...
			 */
			 ...
			 foreach ($notifications as $type){
			 	if (!$this->notification_type_exists($type)){
					...
					$data['status'] = 1;
					$this->save_notification($data);///and save it			 		
			 	}
			 }
		}
		...
		public function notification_type_exists($type=''){
			/*
			 * @param string
			 * @return boolean
			 */
...
<div><strong>Refuse Referrals</strong> : {refuse_referrals}</div>									
		...
		public function affiliates_with_no_rank_exists(){
			 ...
			 */
			global $wpdb;
			$affiliates = $wpdb->base_prefix . 'uap_affiliates';
		    $ranks = $wpdb->base_prefix . 'uap_ranks';
			$q = "SELECT rank_id 
					FROM $affiliates 
					WHERE NOT EXISTS 
					    (SELECT * 
			...
			if (!empty($post_data)){
				...
				if ($this->is_magic_feat_enable('max_amount')){
					...
					if ($max_amount_per_rank && isset($max_amount_per_rank[$rank_id]) && $max_amount_per_rank[$rank_id]!=''){
						...
						if ($post_data['amount']>$max_amount_value){
		...
		public function get_magic_feat_item_list(){
			...
			return array(
					...
					'pushover' => array(
							...
							'extra_class' => '',
							'description' => __('Send User Notification on mobile with Pushover.', 'uap'),
		...
		public function simple_links_save_link($post_data=array(), $status=0){
			...
			if ($post_data && !empty($post_data['affiliate_id']) && !empty($post_data['url'])){
				...
				$table = $wpdb->base_prefix . 'uap_ref_links';
