 ...
 //error_log( print_r(  'ABOVE Cart/catalog test ', true ) );    
  ...
  //error_log( print_r(  '$sizeof_rules_set= ' .$sizeof_rules_set . '(2)= ' . sizeof($vtprd_rules_set). ' $i= ' .$i, true ) );
      ...
      for($k=0; $k < $sizeof_cart_items; $k++) {                 
        ...
        $actionPop_found = $this->vtprd_test_if_actionPop_product($i, $k);   //v2.0.0  added   $actionPop_found 
  ...
  public function vtprd_apply_discount_as_a_group($i, $d, $ar ) {   
    ...
    switch( true ) {
      ...
      case ($vtprd_rules_set[$i]->rule_deal_info[$d]['discount_amt_type']   == 'forThePriceOf_Currency') :
        ...
        //compute remainder
        //$per_unit_savings_2decimals = bcdiv($total_savings , $forThePriceOf_Divisor , 2);
        $per_unit_cost_2decimals = round( ($cart_group_new_fixed_price / $forThePriceOf_Divisor) , 2); 
            
        $running_total =  $per_unit_cost_2decimals * $forThePriceOf_Divisor;
      ...
      //$remainder = $total_savings - $running_total;
        ...
        for ( $s=$vtprd_rules_set[$i]->actionPop_exploded_group_begin; $s < $vtprd_rules_set[$i]->actionPop_exploded_group_end; $s++) {
            //$vtprd_rules_set[$i]->actionPop_exploded_found_list[$s]['prod_discount_amt'] = $per_unit_savings_2decimals;
            $vtprd_rules_set[$i]->actionPop_exploded_found_list[$s]['prod_discount_amt'] = 
                $vtprd_rules_set[$i]->actionPop_exploded_found_list[$s]['prod_unit_price'] - $per_unit_cost_2decimals;
            
            /*
            //add as many pennies as necessary to balance out the remainder
            if ($remainder > 0) {
               $vtprd_rules_set[$i]->actionPop_exploded_found_list[$s]['prod_discount_amt'] += $remainder;
               $remainder -= $penny;
            }
            */
 ...
 //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 006 ', true ) );
     ...
     if ( (sizeof($vtprd_rules_set[$i]->buy_group_population_info['buy_group_groups_excl_array']) > 0) &&
        ...
        if ($this->vtprd_are_groups_in_inPop_list_check($i, $k, 'excl', $vtprd_rules_set[$i]->buy_group_population_info['buy_group_groups_excl_array']) ) {
          $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Groups exclusion found';
 //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 009 ', true ) );
     ...
     if ( (sizeof($vtprd_rules_set[$i]->buy_group_population_info['buy_group_memberships_excl_array']) > 0) &&
        ...
        if ($this->vtprd_are_memberships_in_inPop_list_check($i, $k, 'excl', $vtprd_rules_set[$i]->buy_group_population_info['buy_group_memberships_excl_array']) ) {
          $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'memberships exclusion found';
     ...
     */
     ...
     if (!$customer_found) {     
       if ( (sizeof($vtprd_rules_set[$i]->buy_group_population_info['buy_group_memberships_incl_array']) > 0)  &&
            (function_exists('wc_memberships') ) ) {
          if ($this->vtprd_are_members_in_inPop_list_check($i, $k, 'incl', $vtprd_rules_set[$i]->buy_group_population_info['buy_group_memberships_incl_array']) ) {
 ...
 //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 022 ', true ) );
    ...
    public function vtprd_are_brands_in_list_check($i,$k, $incl_or_excl, $test_this_array) {
      ...
      if (sizeof($vtprd_cart->cart_items[$k]->brands_list) == 0) {
        $vtprd_cart->cart_items[$k]->brands_list = wp_get_object_terms( $vtprd_cart->cart_items[$k]->product_id, $vtprd_cart->cart_items[$k]->brands_tax_found, $args = array('fields' => 'ids') );
      }
    ...
    public function vtprd_are_groups_in_inPop_list_check($i,$k, $incl_or_excl, $test_this_array) {
      global $vtprd_cart, $vtprd_rules_set, $vtprd_rule, $vtprd_info, $vtprd_setup_options;      
       if (sizeof($vtprd_cart->cart_items[$k]->groups_list) == 0) {
          $vtprd_cart->cart_items[$k]->groups_list = do_shortcode('[groups_user_groups]'); //gets current user_id if null
       }
       if (sizeof($vtprd_cart->cart_items[$k]->groups_list) == 0) {
       ...
       if ( array_intersect($vtprd_cart->cart_items[$k]->groups_list, $test_this_array ) ) {
    ...
    public function vtprd_are_memberships_in_inPop_list_check($i,$k, $incl_or_excl, $test_this_array) {
      global $vtprd_cart, $vtprd_rules_set, $vtprd_rule, $vtprd_info, $vtprd_setup_options;      
       if (sizeof($vtprd_cart->cart_items[$k]->memberships_list) == 0) {
          $vtprd_cart->cart_items[$k]->memberships_list = wc_memberships_get_user_active_memberships(); //gets current user_id if null, Returns an array of active memberships plan objects; 
       }
       if (sizeof($vtprd_cart->cart_items[$k]->memberships_list) == 0) {
          $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'User Membership not found in list for '.$incl_or_excl;                  
          return false;
       }
       
       foreach ( $vtprd_cart->cart_items[$k]->memberships_list as $key => $membership ) {
          if (in_array($membership['ID'], $test_this_array)) {
            $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'User Membership found in list for '.$incl_or_excl;                  
            return true;              
          }
       }
...
*/
  ...
  public  function vtprd_get_current_user_roles() {
    ...
    if  ($user_roles[0] <= ' ') {
      ...
      //error_log( var_export($user_roles, true ) );    
