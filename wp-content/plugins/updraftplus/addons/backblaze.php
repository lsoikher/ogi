...
class UpdraftPlus_Addons_RemoteStorage_backblaze extends UpdraftPlus_RemoteStorage_Addons_Base_v2 {
	...
	public function do_upload($file, $local_path) {
		...
		if (!is_object($service)) throw new Exception("Backblaze service error (got a ".gettype($service).")");
	...
	public function chunked_upload($file, $fp, $chunk_index, $upload_size, $upload_start, $upload_end, $total_file_size) {
		...
		if (!is_object($service)) return new WP_Error('no_backblaze_service', "Backblaze service error (got a ".gettype($service).")");
		...
		if (!$large_file_id) {
			...
			try {
				$response = $service->uploadLargeStart(array(
					'FileName'   => $remote_path,
					'BucketName' => $opts['bucket_name'],
				));
		...
		if (!$upload_url || !$auth_token) {
			try {
				...
				$response = $this->storage->uploadLargeUrl(array(
		...
		try {
			$response = $this->storage->uploadLargePart(array(
	...
	public function chunked_upload_finish($file) {
		...
		try {
			$response = $this->storage->uploadLargeFinish(array(
				'FileId' => $this->_large_file_id,
				'FilePartSha1Array' => $this->_sha1_of_parts,
			));
		} catch (Exception $e) {
			global $updraftplus;
			$updraftplus->log("Exception in uploadLargeFinish(): {$e->getCode()}, Message: {$e->getMessage()}");
			return false;
		}
	...
	public function do_delete($file) {
		...
		$result = $this->storage->deleteFile(array(
			'FileName'   => $backup_path.$file,
			'BucketName' => $opts['bucket_name'],
		));
	...
	public function do_listfiles($match = 'backup_') {
		...
		try {
			$remote_files = $this->storage->listFiles(array(
				'BucketName' => $opts['bucket_name'],
	...
	protected function do_credentials_test($testfile, $posted_settings = array()) {
		...
		try {
			...
			if (in_array($bucket_name, $buckets)) {
				...
				// Now try to write
				$result = $this->storage->upload(array(
				...
				echo __('Bucket not found', 'updraftplus')."\n";
			}
	...
	public function do_bootstrap($opts, $connect = true) {
		...
		try {
			...
			$this->storage = new UpdraftPlus_Backblaze_CurlClient($opts['account_id'], $opts['key']);
	...
	public function get_configuration_template() {
		...
		<tr class="<?php echo $classes;?>">
			...
			<em><?php echo sprintf(__('N.B. You need to create the bucket in %s first.', 'updraftplus').' <a href="https://help.backblaze.com/hc/en-us/articles/217666908-What-you-need-to-know-about-B2-Bucket-names">'.__('There are limits upon which path-names are valid. Spaces are not allowed.', 'updraftplus').'</a>', '<a target="_blank" href="https://secure.backblaze.com/b2_buckets.htm">'.__('your Backblaze console', 'updraftplus').'</a>');?></em><br>
			</td>
		</tr>
