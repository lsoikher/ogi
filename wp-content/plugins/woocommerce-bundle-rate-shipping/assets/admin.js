...
( function( $ ) {
    ...
    ENDA_BUNDLERATE.addConfiguration = function(el) {
        var rows = $('#enda-bundle-rate-configurations .enda-configuration'),
            data = {
    ...
    // Add a new layer of configuration
    ENDA_BUNDLERATE.removeConfiguration = function(el) {
        el.parents( '.enda-configuration' ).remove();
    ...
    $(document).ready(function(){   
        ...
        .on('change', 'input.bundle_rate_products', function() {
            ...
            if ( !$(this).val() || '0' === $(this).val() || '' === $(this).val()) {
                $(this).parents('table').first().find('.add_layer').first().attr('disabled', true);
            }
            else { 
                $(this).parents('table').first().find('.add_layer').first().attr('disabled', false);
            }
        })
        .on('click', '.add_bundle_configuration', function() {
            return methods.addConfiguration( $(this) );
        })
        .on('click', '.remove_bundle_configuration', function() {
            return methods.removeConfiguration( $(this) );
        });
    });
})(jQuery);