...
function wc_autoship_ajax_pipey_get_cart() {
	...
	if ( ! empty( $shipping_packages ) && isset( $shipping_packages[0]['rates'] ) ) {
		if ( version_compare( WC()->version, '3.2.0', '<' ) ) {
			...
			// WooCommerce 3.2.X no longer json serializes
			$shipping_method_objects = array_values( $shipping_packages[0]['rates'] );
			$shipping_method_arrays = array();
			foreach ( $shipping_method_objects as $object ) {
				$shipping_method_arrays[] = array(
					'id' => $object->get_id(),
					'label' => $object->get_label(),
					'cost' => $object->get_cost(),
					'taxes' => $object->get_taxes(),
					'method_id' => $object->get_method_id()
				);
			}
			$shipping_methods = $shipping_method_arrays;
		}
	}
