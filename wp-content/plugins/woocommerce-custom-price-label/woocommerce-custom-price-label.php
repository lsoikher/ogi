...
Plugin URI: https://wpcodefactory.com/item/custom-price-labels-for-woocommerce-plugin/
...
final class Woocommerce_Custom_Price_Label {
	...
	function action_links( $links ) {
		...
		if ( 'woocommerce-custom-price-label.php' === basename( __FILE__ ) ) {
			$custom_links[] = '<a href="' . esc_url( 'https://wpcodefactory.com/item/custom-price-labels-for-woocommerce-plugin/' ) . '">' . __( 'Unlock all', 'woocommerce-custom-price-label' ) . '</a>';
		}
	...
	function includes() {
		...
		$settings[] = require_once( 'includes/admin/class-wc-custom-price-label-settings-global.php' );
		$settings[] = require_once( 'includes/admin/class-wc-custom-price-label-settings-local.php' );
		if ( is_admin() && get_option( 'alg_wc_custom_price_label_version', '' ) !== $this->version ) {
