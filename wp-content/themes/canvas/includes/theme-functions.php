	...
	function woo_pagenav( $custom_query = '' ) {
		...
		?>
            <div class="nav-entries">
                <?php next_posts_link( '<span class="nav-prev fl"><i class="icon-angle-left"></i> '. __( 'Older posts', 'woothemes' ) . '</span>', $max_pages ); ?>
				...
				<?php if($mode == 'directions'){ ?>
 				...
 				directions = new GDirections(map, directionsPanel);
			  	...
			  	<?php
	  				...
	  				<?php
	  					// Sanitize string, but still allow some HTML.
	  					$callout = wp_kses_post( str_replace('\'', '\\\'', get_option( 'woo_maps_callout_text' ) ) );
	  					$callout = preg_replace( "/[\n\r]/", "<br/>", $callout );
	  					$callout = preg_replace('#<br[^>]*>(\s*<br[^>]*>)+#', '<br />', $callout);
	  				?>
	  				var callout = '<?php echo $callout; ?>';
